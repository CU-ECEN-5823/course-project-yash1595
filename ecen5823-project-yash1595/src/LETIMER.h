...
// uint32_t Comp0_Cal(float);
 ...
 void BlockWait(uint32_t us_wait);
...
#define Logging()				(float)(RollOver+(0.001*((LETIMER_CounterGet(LETIMER0)*4*1000)/CMU_ClockFreqGet(cmuClock_LFA))))
#define OneClockCycle			(uint32_t)(CMU_ClockFreqGet(cmuClock_LFA)/(CLOCK_DIV))
